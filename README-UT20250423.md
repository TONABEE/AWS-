...
CloudShellを利用する場合、割り当てられるストレージ容量の関係で、以下の手順に従い /tmp配下で実施してください。<br>
CloudShellのホームの容量があまりないので新しくフォルダを作ってAWSを動かすためのものをインストールしてる
...
### 5. CDK スタックのデプロイ
→アプリケーションを立ち上げる<br>
→これを行うとlib配下にあるAWS接続設定ファイルや、bin配下にあるモデルの設定ファイルの設定内容をbootstrapの設定に追加しますよという確認が出てくる
